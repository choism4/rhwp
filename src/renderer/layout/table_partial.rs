...
impl LayoutEngine {
    ...
    pub(crate) fn layout_partial_table(
        ...
        // v5: 어울림(Square) wrap 도 포함 — 타이틀 텍스트 + 표가 같은 문단일 때 표가
        // y_start(문단 시작)에 그려져 타이틀과 겹치는 회귀 차단(씬구성표 MBC). 비-Partial
        // 경로(table_layout.rs)의 vert offset 분기와 동일 정책.
        ...
        for (cell_idx, cell) in table.cells.iter().enumerate() {
                ...
                .enumerate()
            {
                ...
                // [task_dup_render] 대사 중복 디텍터: 분할 행 셀 문단의 줄 범위 덤프(v5).
                // env 미설정 시 무동작. scan_dup.py 가 LRD 라인을 파싱해 구간 겹침 검출.
                // 컷 기반(start_cut/end_cut) 전환으로 offset 컬럼은 디버그용 0 으로 둔다.
