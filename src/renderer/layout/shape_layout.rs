...
// format_number / NumberFormat: v5 inline 쪽번호 양식(Arabic/A,B,C/로마자) 치환에 필요.
use super::super::{format_number, hwpunit_to_px, NumberFormat as NumFmt, PathCommand, ShapeStyle, TextStyle};
    ...
    pub(crate) fn layout_textbox_content(
        ...
        if current_pn > 0 {
            for (pi, para) in text_box.paragraphs[..para_count].iter().enumerate() {
                // v5 ⑦ footer 쪽번호 치환: AutoNumber.format(HWP 표 134) 존중(Arabic/A,B,C/
                // 로마자 등) + marker+작품명 통합 footer(MBC/SBS) 이중삽입 방지(done flag) +
                // 볼드 placeholder run 의 숫자 비볼드 char_shape 분리. fork/main 의 factored
                // helper(substitute_page_auto_numbers_in_composed)는 format byte/볼드 분리를
                // 다루지 않아 회귀하므로, v5 inline 구현을 유지한다.
