...
fn compose_lines(para: &Paragraph) -> Vec<ComposedLine> {
    if para.line_segs.is_empty() {
        ...
        // (1) [Task #994/#998] 공백 기준 word wrap (~45 chars/line) 으로 multi-line 생성 —
        //     PARA_LINE_SEG 누락 HWP3-origin bullet 의 시각 겹침/페이지수 inflate 방지.
        // (2) 각 줄의 runs 는 split_by_char_shapes 로 char_shape 경계대로 분할한다 (v5).
        //     char_shapes.first() 를 줄 전체에 일괄 적용하면 바탕쪽 통합 footer 글상자처럼
        //     한 문단에 여러 char_shape(쪽번호 마커 + 작품명)가 섞일 때 작품명이 마커
        //     char_shape(굵은 바탕·장평 200%)를 잘못 상속한다. (char_shapes 가 비면
        //     split_by_char_shapes 가 내부적으로 split_runs_by_lang 로 폴백한다.)
