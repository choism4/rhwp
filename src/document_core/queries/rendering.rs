...
use crate::model::shape::{ShapeObject, VertAlign, VertRelTo};
...
fn top_master_textbox_bottom_hwpunit(master_page: &MasterPage, page_def: &PageDef) -> Option<i32> {
    let body_top = PageAreas::from_page_def(page_def).body_area.top;
    let mut reserved_bottom: Option<i32> = None;

    for para in &master_page.paragraphs {
        for ctrl in &para.controls {
            if let Control::Shape(shape) = ctrl {
                if let Some(bottom) = top_footer_textbox_bottom_from_shape(shape, body_top) {
                    reserved_bottom = Some(reserved_bottom.map_or(bottom, |v| v.max(bottom)));
                }
            }
        }
    }
...
fn top_footer_textbox_bottom_from_shape(shape: &ShapeObject, body_top: i32) -> Option<i32> {
    let common = shape.common();
    if shape_contains_text_box(shape)
        && common.vert_rel_to == VertRelTo::Paper
        && matches!(common.vert_align, VertAlign::Top | VertAlign::Inside)
    {
        let top = common.vertical_offset as i32;
        if top < body_top {
            return Some(top.saturating_add(common.height as i32));
        }
    }
    ...
    if let ShapeObject::Group(group) = shape {
        group
            .children
            .iter()
            .filter_map(|child| top_footer_textbox_bottom_from_shape(child, body_top))
            .max()
    } else {
        None
    }
...
fn shape_contains_text_box(shape: &ShapeObject) -> bool {
    if shape
        .drawing()
    ...
    matches!(shape, ShapeObject::Group(group) if group.children.iter().any(shape_contains_text_box))
}
...
impl DocumentCore {
    ...
    fn reserve_body_for_top_master_footer(
        page: &mut crate::renderer::pagination::PageContent,
        page_def: &PageDef,
        ...
        for mp_ref in page
            .active_master_page
            .iter()
            .chain(page.extra_master_pages.iter())
        {
            if let Some(mp) = master_pages.get(mp_ref.master_page_index) {
                if let Some(bottom) = top_master_textbox_bottom_hwpunit(mp, page_def) {
                    reserved_bottom = Some(reserved_bottom.map_or(bottom, |v| v.max(bottom)));
                }
            }
        }

        let Some(reserved_bottom) = reserved_bottom else {
            return;
        };

        page.layout.reserve_body_top_until_hwpunit(reserved_bottom);
        for col in &mut page.column_contents {
            if let Some(zone_layout) = &mut col.zone_layout {
                zone_layout.reserve_body_top_until_hwpunit(reserved_bottom);
            }
        }
    }
        ...
        for (idx, section) in self.document.sections.iter().enumerate() {
            ...
            if !section.section_def.master_pages.is_empty() {
                ...
                for (page_idx_in_section, page) in result.pages.iter_mut().enumerate() {
                    ...
                    Self::reserve_body_for_top_master_footer(
                        page,
                        &section.section_def.page_def,
                        mps,
                    );
